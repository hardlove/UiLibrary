...
public class SectionItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        int pos = ((LinearLayoutManager) (Objects.requireNonNull(parent.getLayoutManager()))).findFirstVisibleItemPosition();
        if (pos < 0) return;
        if (mData == null || mData.isEmpty()) return;
        String section = mData.get(pos).getSection();
        View child = Objects.requireNonNull(parent.findViewHolderForLayoutPosition(pos)).itemView;
